...
const supplierController = {
    ...
    getAll: async (req, res) => {
        ...
        try {
            ...
            const activeSuppliers = await Supplier.findAll({
                where: whereActive,
                order: [['name', 'ASC']] // Corrigido para 'name'
            });
    ...
    create: async (req, res) => {
        const { tenantId } = req;
        const { name, contactPerson, phoneNumber, email, address } = req.body; // MUDANÇA AQUI: companyName → name
        ...
        try {
            ...
            if (!name || name.trim() === '') {
                return res.redirect('/fornecedores?error=name_required');
            }
            
            await Supplier.create({
                name: name.trim(), // MUDANÇA AQUI: companyName → name
                contactPerson,
                phoneNumber,
                email,
                address,
            ...
            if (error.name === 'SequelizeValidationError') {
                ...
                return res.redirect(`/fornecedores?error=validation_error&details=${encodeURIComponent(errorMessage)}`);
            }
            res.redirect(`/fornecedores?error=${error.message || 'create_failed'}`);
        }
    },
...
update: async (req, res) => {
    const { tenantId } = req;
    const { id } = req.params;
    const { name, contactPerson, phoneNumber, email, address, status } = req.body;
    
    console.log('🔍 DEBUG Update - Parâmetros:', { id, name, contactPerson }); // DEBUG
